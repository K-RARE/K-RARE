...
# TopK config (a doctor checks at most the top K candidate diagnoses)
TOPK = 3
...
def grid_search_knn_classifier(X_train, y_train, param_grid, cv_folds=3, rnd_state=42, target_name=""):
    skf = StratifiedKFold(n_splits=cv_folds, shuffle=True, random_state=rnd_state)
    grid = []
    for n_neighbors in param_grid.get("n_neighbors", [5]):
        for weights in param_grid.get("weights", ["uniform"]):
            grid.append({"n_neighbors": int(n_neighbors), "weights": weights})

    best_score = -1.0
    best_params = None

    print(f"\n[GridSearch-KNN-{target_name}] Total combinations: {len(grid)}, CV={cv_folds}, metric=macro-F1")
    for i, params in enumerate(grid, 1):
        f1_fold = []
        for _, (tr_idx, va_idx) in enumerate(skf.split(X_train, y_train), 1):
        ...
        mean_f1 = float(np.mean(f1_fold))
        print(f"  ({i}/{len(grid)}) params={params}  macro-F1(CV)={mean_f1:.4f}")
        if mean_f1 > best_score:
            best_score = mean_f1
            best_params = params

    print(f"[GridSearch-KNN-{target_name}] Best params: {best_params}, best CV macro-F1={best_score:.4f}")
...
def run_knn_for_task(task: dict):
    ...
    if ENABLE_GRID_SEARCH:
        best_params, best_cv_score = grid_search_knn_classifier(
            X_train, y_train,
            PARAM_GRID_KNN,
            cv_folds=CV_FOLDS,
        ...
        best_cv_score = None
        print(f"[GridSearch-KNN-{name}] Grid search disabled; using default params: {best_params}")

    os.makedirs(RUN_DIR, exist_ok=True)
    best_param_path = os.path.join(RUN_DIR, f"{name}_knn_best_params.json")
    ...
    print(f"[{name}] Best parameters saved to: {best_param_path}")
