...
LLM_OUTPUT_COL_NAME = "biomistral_7b_111"
...
TASKS = [
    {
        ...
        "output_root": Path("results/LLM_With_Rule_y3/pressure_low"),
        "output_csv_name": "pressure_low_llm_outputs.csv",
        "accuracy_txt_name": "pressure_low_accuracy_results_biomistral_111.txt",
    },
    {
        "name": "pressure_high",
        ...
        "output_root": Path("results/LLM_With_Rule_y3/pressure_high"),
        "output_csv_name": "pressure_high_llm_outputs.csv",
        "accuracy_txt_name": "pressure_high_accuracy_results_biomistral_111.txt",
    },
]
...
def save_llm_outputs_to_csv(predictions, csv_path: Path, json_key: str):
    ...
    if csv_path.exists():
        ...
        if "patient_index" in df_exist.columns:
            df_exist["patient_index"] = df_exist["patient_index"].astype(int)
            df_pred["patient_index"] = df_pred["patient_index"].astype(int)
...
def update_accuracy_txt(json_key: str, matches: int, total_used: int, acc: float, acc_file: Path):
...
def run_task(task, model, tokenizer):
    ...
    print("\n" + "=" * 80)
    print(f"Task: {name} (BioMistral-7B, LLM_With_Rule_y3)")
    ...
    if gold_col not in df.columns:
        raise KeyError(f"[{name}] Cannot find column '{gold_col}' in the test set. Current columns: {list(df.columns)}")
    ...
    update_accuracy_txt(json_key, matches, total_used, acc, acc_file=accuracy_txt_path)
